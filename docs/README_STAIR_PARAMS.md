...
| `roi_v_start` | float | 0.5 | 垂直方向 ROI 起始位置 (0.0 - 1.0)，占图像高度的百分比。 |
| `roi_v_stop` | float | 1.0 | 垂直方向 ROI 结束位置 (0.0 - 1.0)。 |
| **图像预处理** |
...
| `wall_iqr_th` | float | 0.05 | 墙壁“平整度”阈值 (米)。计算 ROI 有效深度的分位距 IQR(P75-P25)，若 IQR 小于该值才允许判定为墙（用于降低近距离台阶被误判为墙）。 |
