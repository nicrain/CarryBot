...
\begin{itemize}
    \item<1-> Rigidité et alignement du mécanisme Tri-Star
    \item<2-> Résistance au pivot (nombre de roues élevé)
    \item<3-> Détection arrière limitée en montée
    \item<4-> Visibilité insuffisante en descente
    \item<5-> Ports moteurs saturés (ajout vérin)
        \par {\small\textcolor{green!45!black}{$\Rightarrow$ Pilotage vérin via pont en H L293D}}
    \item<6-> Alimentation Raspberry Pi inadaptée
        \par {\small\textcolor{green!45!black}{$\Rightarrow$ UPS Shield + 4 batteries 18650 (5A stable)}}
    \item<7-> CarryBot ne montait pas avec un moteur de monte
        \par {\small\textcolor{green!45!black}{$\Rightarrow$ Ajout d’un second moteur arrière}}
    \item<8-> Assemblage chassis impossible a imprimer en 3D
